...
- 模块化不是把文件切碎，而是让每个部件只承载一小段清楚职责，并且能稳定接入整体。
...
- 对 `koko` 来说，adapter、repository、同步链路、事件载体都应先守同一表面，再允许不同实现并列存在，而不是每遇到一个场景就补一个私有 helper。
...
- 对 `koko` 来说，`command / snapshot / event / error` 应优先保持稳定、清楚、可观察、可串联；不要让信息一进某层就变成只有当前壳或当前 adapter 才看得懂的私有对象。
