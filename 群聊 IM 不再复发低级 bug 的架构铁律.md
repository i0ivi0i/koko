...
这份设计不讨论某个具体 bug 该怎么补，而是给 `koko` 的群聊 IM 主链路立一组长期有效的铁律。  
目标只有一个：以后再遇到“点发送没反应”“明明进房却显示未连接”“消息已入库却没成立”这类低级错时，系统能更早拦住、更快定位，而不是让问题在多层之间来回漂。
...
- 真正强的系统，不是每个部件都很神，而是普通部件也能靠清楚关系稳定拼出大能力。

### 6. 限制耦合方式，放大组合数量

- 系统真正的杠杆，不来自接口花样越来越多，而来自少数几种稳定连接方式被反复复用。
- `koko` 应尽量把跨层连接收口到少数清楚通道：`command / query / snapshot / event / error code / event_position`，不要为了局部方便再长出特例入口、旁路回调和隐式共享协议。

### 7. 组件要做成可替换家族，不做散装 helper

- 成熟系统依赖的不是一堆一次性小函数，而是一组围绕同一契约展开、可替换、可裁剪、可并列演进的组件家族。
- 对 `koko` 来说，adapter、repository、同步链路、事件通道都应先守同一表面，再允许不同实现并列存在，而不是每遇到一个场景就补一个私有 helper。

### 8. 名字要稳定，绑定要可变

- 核心能力应该先拥有稳定名字和稳定语义，具体绑定到 Web、CLI、iOS、Android、HTTP、realtime 还是本地实现，应该是外层适配问题。
- 不要把某个壳、某个协议、某次临时接线写死进核心命名里；名字一旦跟着入口乱漂，系统就会逐渐失去可替换性。

### 9. 少机制种类，少例外入口

- 机制种类一多，系统解释成本、测试面和 bug 面积都会一起长大。
- 能靠已有契约表达的能力，不新增新机制；能走已有入口的链路，不新增例外入口；能删掉一个特判，就不要把它升级成长期结构。

### 10. 先选公共交换形态，再谈局部最优格式

- 真正能放大系统能力的，不只是模块本身，而是模块之间能否沿同一种公共形态顺畅交换信息。
- 对 `koko` 来说，`command / snapshot / event / error` 应优先保持稳定、清楚、可观察、可串联；不要让信息一进某层就变成当前壳或当前 adapter 专用的私有对象。
...
- 设计跨层载荷时，优先选可观察、可回放、可补洞、可串联的公共表达；不要为了某一层眼前省事，把信息压成外人看不懂的私有形状。
- 拿不准设计时，先退回更直接、更清楚、机制更少的方案；不要在证据不足时提前上复杂做法。
- 新增字段、接口或事件前，默认先搜现有契约和数据流；能复用就复用，不能因为眼前方便再开第二入口。
- 遇到重复分支、重复映射、重复同步胶水时，优先收口成一处稳定实现，不靠复制粘贴维持一致。
- 遇到一个 helper 只服务单一分支、单一页面、单一入口时，默认先怀疑这是不是该并回稳定契约，而不是继续累积散装工具。
- 优先把变化编码成数据、表和显式映射，再考虑堆条件分支；分支越多，真相越容易被悄悄复制。
- 设计名字时，把语义名和绑定名分开；`join_room`、`message_created` 这种名字应表达事实，不应把 `http_`、`ws_`、`web_` 这类外层绑定写死进核心。
- 调试链路时，优先沿主信息流追证据：命令如何入、事件如何出、快照如何接、错误如何露；不要依赖只存在于某个页面或某个连接态里的旁路线索。
...
- 真正不优雅的不是“用了 HTTP”，而是把冷路径和热路径炖成一锅。
...
这三条链可以共享契约，但不能混成一个“连上了所以一切都对”的假状态机。
...
以后真正要防的，不是“某个按钮偶尔点不动”，而是系统重新滑回“多层都能宣布真相”的旧路。  
