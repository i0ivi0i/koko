...
## Corpus Check
- 220 files · ~268,951 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 530 nodes · 690 edges · 32 communities detected
...
1. `创建房间HTTP接口()` - 13 edges
2. `Invoke-HttpsBootstrap()` - 10 edges
3. `单连接发送到已关闭socket时降级为正常断开()` - 9 edges
4. `假Hls构造器` - 7 edges
5. `提取媒体上传授权头()` - 7 edges
6. `Start-AppViaRunScriptIfNeeded()` - 6 edges
7. `update_status()` - 6 edges
8. `load_streaming_asset_content()` - 6 edges
9. `假Socket` - 5 edges
10. `预制图片为CanonicalWebp()` - 5 edges
...
Cohesion: 0.03
Nodes (59): abandon_media_upload(), AbandonMediaUploadBody, ApiError, axum_ws_message_to_tungstenite(), Blob媒体资产响应参数, BootstrapBody, Canonical图片校验结果, 媒体complete测试环境 (+51 more)

### Community 1 - "Community 1"
Cohesion: 0.03
Nodes (44): 插入0018前脏匿名身份与视频附件(), 创建资产协作分发Actor(), AdminLoginBody, AdminLoginResp, 新模块会拒绝非canonical_webp字节(), 预制图片为CanonicalWebp(), dispatch(), dispatchPointerScrollIntent() (+36 more)
...
Cohesion: 0.1
Nodes (3): handleSummary(), 创建房间HTTP接口(), summarizeTrend()
...
Cohesion: 0.17
Nodes (9): Assert-ServicesStopped(), Clear-CleanupTarget(), Clear-DirectoryContents(), Get-ListeningPortProcessRecords(), Get-StartupArtifactOptimizationTargets(), Resolve-ManagedPath(), Resolve-RecognizedProjectService(), Stop-RecognizedProjectServices() (+1 more)
...
- **Thin community `Community 26`** (1 nodes): `css.d.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 27`** (1 nodes): `blob媒体资产测试.spec.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 28`** (1 nodes): `vitest.config.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 29`** (1 nodes): `lib.rs`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 30`** (1 nodes): `up.ps1`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 31`** (1 nodes): `webtorrent.d.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `单连接发送到已关闭socket时降级为正常断开()` connect `Community 5` to `Community 0`, `Community 1`, `Community 10`?**
  _High betweenness centrality (0.046) - this node is a cross-community bridge._
- **Why does `提取媒体上传授权头()` connect `Community 0` to `Community 1`, `Community 2`, `Community 15`?**
