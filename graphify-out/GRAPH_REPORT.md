...
## Corpus Check
- 195 files · ~232,948 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 454 nodes · 569 edges · 31 communities detected
...
3. `假Hls构造器` - 7 edges
4. `提取媒体上传授权头()` - 7 edges
5. `update_status()` - 6 edges
6. `load_streaming_asset_content()` - 6 edges
7. `假Socket` - 5 edges
8. `run_k6_level()` - 5 edges
9. `handle_tus_hook()` - 5 edges
10. `parse_attachment_content_query()` - 5 edges
...
Cohesion: 0.03
Nodes (49): abandon_media_upload(), AbandonMediaUploadBody, ApiError, Blob媒体资产响应参数, BootstrapBody, 打包产物上传会并发执行而不是串行阻塞complete热路径(), complete_media_upload(), CompleteMediaUploadBody (+41 more)
...
Cohesion: 0.05
Nodes (20): dispatch(), dispatchPointerScrollIntent(), dispatchScrollIntent(), dispose(), exitCurrentRoomView(), 协作分发JoinTicket失效错误, leaveCurrentRoomView(), 看起来像Promise() (+12 more)
...
Cohesion: 0.07
Nodes (18): 创建资产协作分发Actor(), AdminLoginBody, AdminLoginResp, 尝试加载dotenv(), 读取exif方向(), flush(), 签发协作分发join_ticket(), make_writer() (+10 more)
...
- **Thin community `Community 21`** (2 nodes): `图片资产描述包含_preview_full_original_而不是普通附件直链()`, `blob媒体资产契约测试.rs`
...
- **Why does `提取媒体上传授权头()` connect `Community 0` to `Community 16`, `Community 1`, `Community 2`, `Community 4`?**
