...
## Corpus Check
- 230 files · ~296,764 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 596 nodes · 1261 edges · 14 communities detected
- Extraction: 63% EXTRACTED · 37% INFERRED · 0% AMBIGUOUS · INFERRED: 464 edges (avg confidence: 0.8)
...
Cohesion: 0.04
Nodes (113): 原始冷源超过24小时后会被后台清理并写入删除时间(), abandon会先写业务abandoned再协调官方termination(), abandon_media_upload(), 放弃媒体上传会清掉当前会话下所有partial临时文件(), 放弃媒体上传会同时标记附件与transport为abandoned并清掉已登记的临时文件(), 启动假tus_termination侧车(), active_backend_strong_seed会让同swarm过期附件保持ready(), announce_url() (+105 more)

### Community 1 - "Community 1"
Cohesion: 0.03
Nodes (86): 插入0018前脏匿名身份与视频附件(), 是64位小写hex(), AbandonMediaUploadBody, admin_login(), admin_overview(), admin_room_detail(), admin_rooms(), AdminLoginBody (+78 more)

### Community 2 - "Community 2"
Cohesion: 0.04
Nodes (56): 备份并清空环境变量(), 构造统一媒体文件Accept(), acquire_timeout(), 创建资产协作分发Actor(), 向受控页面广播后台补发请求(), 新模块会拒绝非canonical_webp字节(), 预制图片为CanonicalWebp(), connectedCallback() (+48 more)

### Community 3 - "Community 3"
Cohesion: 0.04
Nodes (15): buildAttachmentContentUrl(), createFakeStorage(), createSocket(), 读取做种监听器预算(), 归一化InfoHash(), 读取JoinTicket(), main(), 读取WebTorrent构造器() (+7 more)

### Community 4 - "Community 4"
Cohesion: 0.11
Nodes (16): Ensure-FrontendDependenciesInstalled(), Get-ListeningPortProcessRecords(), Invoke-LauncherCleanup(), New-ManagedProcess(), New-StreamState(), Read-NewLogLines(), Resolve-PwshPath(), Resolve-RecognizedLauncherService() (+8 more)
...
- **Are the 27 inferred relationships involving `备份并清空环境变量()` (e.g. with `创建房间HTTP接口()` and `get()`) actually correct?**
  _`备份并清空环境变量()` has 27 INFERRED edges - model-reasoned connections that need verification._
