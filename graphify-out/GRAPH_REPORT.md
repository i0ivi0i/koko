...
## Corpus Check
- 72 files · ~1,690,493 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 251 nodes · 337 edges · 28 communities detected
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 4 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output

## God Nodes (most connected - your core abstractions)
1. `send_json()` - 24 edges
2. `HttpRealtime传输` - 22 edges
3. `构造rustus_hook请求体()` - 9 edges
4. `写入rustus测试文件()` - 8 edges
...
9. `假Socket` - 5 edges
10. `post_finish稍后到达时complete媒体上传会等待回执并成功()` - 5 edges
...
Cohesion: 0.08
Nodes (33): 房间历史分页会返回before_event_position之前的消息(), 房间历史分页缺少before_event_position会返回invalid_argument(), bootstrap接口会返回稳定花名快照(), 没有上传回执时complete媒体上传会返回attachment_not_ready(), complete图片上传遇到非图片原图会返回attachment_type_not_allowed(), complete图片上传会把prepared附件升级成ready并写入缩略图(), 非成员不能通过events接口拉取房间增量(), 成员通过events接口只会拿到from之后的事件() (+25 more)
