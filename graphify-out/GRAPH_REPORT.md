...
## Corpus Check
- 57 files · ~116,250 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 192 nodes · 242 edges · 27 communities detected
...
Cohesion: 0.12
Nodes (17): 非图片上传会返回attachment_type_not_allowed(), 房间历史分页会返回before_event_position之前的消息(), 房间历史分页缺少before_event_position会返回invalid_argument(), bootstrap接口会返回稳定花名快照(), 非成员不能通过events接口拉取房间增量(), 成员通过events接口只会拿到from之后的事件(), 不存在的房间通过events接口会返回room_not_found(), 非成员不能通过history接口读取更早消息() (+9 more)
...
## Knowledge Gaps
- **20 isolated node(s):** `frontend/存储.ts`, `frontend/房间内核.ts`, `frontend/房间恢复编排.ts`, `frontend/房间滚动器.ts`, `文本布局` (+15 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **Thin community `Community 9`** (2 nodes): `文本布局`, `文本布局测试`
...
- **Thin community `Community 23`** (1 nodes): `frontend/阅读推进编排.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 24`** (1 nodes): `输入区布局测试`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
