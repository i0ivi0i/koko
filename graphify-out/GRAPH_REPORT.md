...
Cohesion: 0.09
Nodes (31): 房间历史分页会返回before_event_position之前的消息(), 房间历史分页缺少before_event_position会返回invalid_argument(), bootstrap接口会返回稳定花名快照(), 等待complete所需运输回执(), 没有上传回执时complete媒体上传会返回attachment_not_ready(), complete图片上传遇到非图片原图会返回attachment_type_not_allowed(), complete_media_upload(), complete图片上传会把prepared附件升级成ready并写入缩略图() (+23 more)
