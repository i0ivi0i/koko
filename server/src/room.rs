...
mod tests {
    ...
    async fn 创建房间时应持久化短码与群主关系(pool: PgPool) {
        ...
        sqlx::query!("INSERT INTO profiles (id, device_key) VALUES ($1, $2)", profile_id.0, "device-1")
            .execute(&pool)
            .await
            .unwrap();
        ...
        let room_code = sqlx::query_scalar!("SELECT code FROM room_codes WHERE room_id = $1", room.id.0)
            .fetch_one(&pool)
            .await
            .unwrap();
