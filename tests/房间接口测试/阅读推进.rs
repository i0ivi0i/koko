...
async fn 阅读锚点会写入当前匿名身份与房间的唯一记录() {
    ...
    let (identity_id, room_id) = tokio::task::spawn_blocking(move || {
        ...
        koko::room::application::推进房间阅读位置(&mut repo, &room_id, &identity.会话标识, 0)
            .expect("应能写入初始阅读锚点");
        (identity_id, room_id)
    })
    .await
...
async fn 阅读锚点只会单调前进不会被回退覆盖() {
    ...
    let room_id = tokio::task::spawn_blocking(move || {
        ...
        koko::room::application::推进房间阅读位置(&mut repo, &room_id, &identity.会话标识, 5)
            .expect("应能先写入更靠后的位置");
        koko::room::application::推进房间阅读位置(&mut repo, &room_id, &identity.会话标识, 2)
            .expect("较早位置不应破坏已有锚点");
        room_id
    })
    .await
