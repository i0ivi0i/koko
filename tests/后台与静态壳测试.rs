...
async fn http冷路径成功会输出accepted与succeeded日志() {
    ...
    assert!(
        output.contains("application") && output.contains("引导匿名身份"),
        "成功主链日志缺少 application: {output}"
    );
    assert!(
...
async fn http冷路径拒绝会输出rejected日志与error_code() {
    ...
    let output = 读取日志缓冲(&buffer);
    assert!(
        output.contains("application") && output.contains("管理员登录"),
        "拒绝日志缺少 application: {output}"
    );
    assert!(
...
#[tokio::test]
#[serial]
async fn 根作用域服务工作线程会同源暴露且显式声明根scope() {
    let cfg = koko::assembly::读取配置().expect("需要本地 DATABASE_URL");
    let state =
        koko::shell::构建应用状态(cfg.database_url.clone(), cfg.admin_password.clone())
            .await
    ...
    for sw_uri in ["/app-sw.js", "/media-sw.js"] {
        let response = app
            .clone()
            .oneshot(
                Request::builder()
        ...
        assert_eq!(response.status(), StatusCode::OK, "{sw_uri} 应可读取");
        assert_eq!(
            response
                .headers()
                .get(header::CACHE_CONTROL)
