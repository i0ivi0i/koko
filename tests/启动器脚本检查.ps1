...
Assert-True ($runScript -match 'Test-InteractiveConsoleHost') "run.ps1 应该先判断当前宿主是否支持交互式清理菜单，避免无头宿主直接死在 RawUI。"
Assert-True ($runScript -match 'IsInputRedirected|IsOutputRedirected|IsErrorRedirected') "run.ps1 的交互宿主判断应覆盖输入/输出重定向，避免 Clear-Host 在无控制台句柄宿主里直接炸掉。"
...
Assert-True ($cleanupMenuSection -match 'if \(-not \(Test-InteractiveConsoleHost\)\)') "run.ps1 的清理模式菜单应先判断宿主是否支持交互式 RawUI。"
Assert-True ($cleanupMenuSection -match '自动继续默认轻清理') "run.ps1 在无交互宿主里应明确提示已自动退回默认轻清理。"
Assert-True ($cleanupMenuSection -match 'return "optimize"') "run.ps1 在无交互宿主里应自动回到默认轻清理，而不是阻塞或直接报错。"
