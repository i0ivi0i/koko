...
Assert-True (-not ($runScript -match 'Cloudflare|cloudflare|cloudflared|trycloudflare|DisableCloudflareTunnel|CLOUDFLARE_TUNNEL')) "run.ps1 已由 https.ps1 承接 HTTPS，不得继续保留 Cloudflare tunnel 启动、下载、解析或开关。"
...
Assert-True (-not ($cleanScript -match 'Cloudflare|cloudflare|cloudflared|trycloudflare')) "qingli.ps1 不得继续保留 Cloudflare 退场后的临时目录或清理入口。"
