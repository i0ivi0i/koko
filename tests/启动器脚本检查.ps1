...
Assert-True ($runScript -match 'concatenation') "run.ps1 必须把 concatenation 当成显式可见的运行时能力，而不是把大视频并行分片建立在隐式默认上。"
Assert-True ($runScript -match 'creation-with-upload') "run.ps1 显式钉住 tus extensions 时，应该同时保留 creation-with-upload，避免把一次请求内完成的上传语义悄悄砍掉。"
Assert-True ($runScript -match 'RUSTUS_REMOVE_PARTS') "run.ps1 应允许显式覆写 remove-parts，避免 final concat 成功后 residual partial 永远靠人工清。"
Assert-True ($runScript -match '--remove-parts') "run.ps1 应能在启用时把 remove-parts 传给 rustus。"
