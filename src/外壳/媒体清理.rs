use super::{tus_hook外壳, 应用状态, 构建共享仓储};
...
/// 执行一次媒体冷源清理：
/// 1. 应用层先给出“哪些图片原图 / 视频回退母本该删了”；
/// 2. 壳层真正删除对象存储里的短期回退对象；
/// 3. 删除成功后再把删除时间回写到附件真相。
///
/// 这里故意不再接手 attachment_streaming_manifests：
/// - 新附件正式主链已经不再写这张表；
/// - 旧 manifest 残留若还存在，只能作为独立 legacy 债务后续迁移，不得继续挂在当前生产 owner 上；
/// - 这样冷源清理 owner 才不会一边宣称“纯 WebTorrent”，一边继续维护第二条视频主链遗物。
///
