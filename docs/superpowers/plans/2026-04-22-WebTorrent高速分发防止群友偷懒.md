...
- [ ] **Step 1: 写失败测试，要求后端契约暴露稳定媒体状态 code，而不是只给 `available/expired`**
...
- [ ] **Step 5: 在 Rust 契约中引入稳定状态结构，并禁止新附件继续依赖旧直链字段作为正式真相**
...
- [ ] **Step 6: 在前端契约中同步新增状态 code，并把旧 `availability` 降级为兼容字段而非主判断字段**
...
- [ ] **Step 1: 写失败测试，要求 `complete` 时写入权威 `complete_at`，而不是由前端或首播时刻决定**
...
- [ ] **Step 3: 写失败测试，要求服务重启或清理流程不会重置退场基点**

```rust
assert_eq!(loaded.retire_at, Some(fixed_complete_at + Duration::hours(24)));
```

- [ ] **Step 4: 运行冷源清理相关测试确认先红**

Run: `cargo test --test 媒体后台测试 冷源清理 -- --nocapture`  
Expected: FAIL，清理逻辑仍基于旧时间字段或重新推导

- [ ] **Step 5: 在 `complete_media_upload` 中落权威 `complete_at` / `retire_at`，并显式区分未 complete / 已 abandon / 已 delete**
...
- [ ] **Step 6: 在清理逻辑里只消费持久化退场时间，不再用前端访问行为重算**
...
- [ ] **Step 1: 写失败测试，要求新附件 locator 在 `24 小时` 前后都只围绕 swarm 真相投影**
...
- [ ] **Step 2: 运行协作分发投影测试确认先红**

Run: `cargo test --test 协作分发测试 投影一致性 -- --nocapture`  
Expected: FAIL，locator 仍会为新附件带出旧 `HLS/thumbnail/original` 正式链字段

- [ ] **Step 3: 写失败测试，要求 `24 小时` 后无在线完整 peer 时返回 `MEDIA_CONNECTING_TO_PEERS -> MEDIA_NO_ONLINE_SEED` 的权威结果**
...
- [ ] **Step 6: 在 `load_media_locator` 中只为 legacy 附件保留旧兼容字段，新附件正式投影只围绕分发片段与状态 code**
...
- [ ] **Step 1: 写失败测试，要求时间线和查看器共用同一 swarm session / file handle 真相**
...
- [ ] **Step 6: 在媒体播放与查看器中移除新附件的第二正式链选择，只允许 legacy 走旧兼容分支**
...
- [ ] **Step 1: 写失败测试，要求无本地缓存的新图片不会偷偷去拿服务器 thumbnail**
...
- [ ] **Step 6: 在存储运行时中区分“24小时退场不清本地”与“删除必须 purge”**
...
- [ ] **Step 1: 写失败测试，要求启动脚本把 `WebRTC seeder` 当成正式受管进程，而不是人工附属脚本**
...
- [ ] **Step 2: 运行启动器脚本检查确认先红**

Run: `pwsh -File tests/启动器脚本检查.ps1`  
Expected: FAIL，当前识别服务列表还没有 `webtorrent-seeder`

- [ ] **Step 3: 写失败测试，要求后端在 `complete` 后登记 sidecar 做种 intent，在退场或删除时发出 stop/reconcile**
...
- [ ] **Step 5: 写最小 sidecar，基于成熟 WebTorrent/WebRTC 能力暴露 `seed/start/stop/reconcile` 薄命令面**
...
- [ ] **Step 6: 在 Rust 后端中登记 seeding intent，并在 `complete / retire / delete / startup reconcile` 驱动 sidecar**
...
- [ ] **Step 7: 在 `run.ps1` 中把 seeder 纳入受管启动、日志、清理与 HTTPS 联机链路**

```powershell
New-ManagedProcess -Name "webtorrent-seeder" -Command $SeederCommand -ReadyPattern "Seeder ready"
```

- [ ] **Step 8: 重新运行启动器脚本检查与后端 seeding intent 测试并确认转绿**
...
- [ ] **Step 1: 写失败测试，要求 `MEDIA_CONNECTING_TO_PEERS` 先显示连接群友，而不是通用加载中**

```ts
expect(view.textContent).toContain("正在尝试连接群友");
```

- [ ] **Step 2: 写失败测试，要求 8 秒预算耗尽后转成 `MEDIA_NO_ONLINE_SEED`**
...
- [ ] **Step 3: 写失败测试，要求删除态覆盖本地缓存与 peer 恢复尝试**
...
- [ ] **Step 5: 在运行时中落地 8s / 2s / 15s 的默认重试窗口，并禁止 deleted 再进入 peer 连接循环**
...
- [ ] **Step 6: 在查看器与时间线中统一消费 contract code，而不是各自派生另一套状态机**
...
- [ ] `& 'E:\koko\https.ps1'`
- [ ] Chrome DevTools 双会话烟测：房间 `1234b`、图片 + MP4、退场前后、无在线种子、删除终态
- [ ] `git status --short`
- [ ] `python3 -c "from graphify.watch import _rebuild_code; from pathlib import Path; _rebuild_code(Path('.'))"`（代码有改动时）

---

Plan complete and saved to `docs/superpowers/plans/2026-04-22-WebTorrent高速分发防止群友偷懒.md`.
