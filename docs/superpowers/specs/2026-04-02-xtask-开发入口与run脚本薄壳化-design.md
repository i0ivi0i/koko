...
- `xtask` 只负责把外部输入转成开发期编排动作
- 配置默认值、配置文件真相、启动地址派生、管理员口令真相仍全部留在 Rust 主程序

也就是说，`xtask` 可以接收 `--database-url` / `--admin-token` / `--bind-addr`，但它不能：
...
为了让实现没有歧义，本轮直接规定 `xtask` 的最小可测结构必须至少拆出三类内部依赖注入口：

- `CommandRunner`：负责 `cargo` / `psql` / bundle 脚本调用
- `ChildProcessLauncher`：负责启动 Rust 主程序并提供 stdout 读取
- `BrowserOpener`：负责 auto-open

它们可以是 trait、闭包包裹器或小型结构体，但必须满足：

- 正式 CLI 只走真实实现
- 单测可以注入 fake toolchain / fake child / fake browser
- 不需要再把测试 seam 暴露成 `run.ps1` 参数或 xtask 的正式用户参数
...
1. 先在 xtask 侧补齐等价或更强的回归覆盖
2. 再删除 `run.ps1` 对应 seam 和耦合测试
