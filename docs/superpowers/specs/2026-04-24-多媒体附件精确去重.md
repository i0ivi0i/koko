...
预检只允许在当前会话有发送权限的目标上下文里产生结果。
它不是“查全站是否存在某文件”的接口，而是“如果允许复用，就直接创建当前业务附件引用”的入口。

禁止把预检接口做成“只返回是否存在”的全局 hash 查询；命中必须服务当前新附件成立。

预检允许使用的复用来源包括：

1. 当前身份自己已经上传并仍有权复用的相同 `source_hash` 资产；
2. 当前身份当前可见消息中已经存在的相同 `source_hash` 资产；
3. 当前身份通过转发、分享或未来资产授权链获得复用权的资产；
4. 当前目标房间允许新建消息和附件引用。
...
**命中后新增的是当前房间的新附件引用；被复用的是全局 canonical 资产。**

禁止把命中结果解释成“复用旧消息”或“复制旧附件事实”。
...
禁止因为 `source_hash` 未命中就为相同 canonical 字节创建第二份物理资产。
...
禁止把未来“转发 / 分享”设计成“证明自己持有原文件”的上传逻辑。

转发时客户端不需要有原始文件，也不需要重新计算 `source_hash`。
它应该提交：
...
裁决成立后，后端创建目标房间的新消息和新附件引用，并绑定同一全局 canonical 资产。

转发不复制媒体字节，不重新生成 torrent，不重新创建 swarm，也不要求客户端持有原文件。

禁止把转发当成重新上传；转发只能创建新的消息和附件引用。
...
禁止把三条链合并成一个万能 `source_hash` 流程，也禁止让转发路径依赖原文件字节。
...
6. 把转发 / 分享误实现成重新上传或要求客户端持有原文件。
7. 把全局资产池误实现成全局可探测 hash 字典。
...
5. 转发 / 分享必须以源附件可见性为授权来源，而不是以 `source_hash` 为授权来源。
...
4. 同一 canonical 内容必须进入同一 swarm，禁止因为业务附件不同而制造重复 torrent。
5. 转发和分享只能增加业务引用，禁止制造第二个 torrent 或第二个 swarm。
...
2. 它属于 swarm 分发平面。
3. 禁止让它变回第二正式播放主链。
4. 禁止用 `source_hash` 去重改变 `24 小时` 服务器退字节裁决。
...
1. Video.js 只能做播放器壳。
2. 禁止播放器壳拥有媒体身份、附件权限、删除真相和 swarm ready 真相。
3. 若未来升级到 Video.js v10，必须通过播放器 adapter 接入。
4. 禁止为了换播放器重写 WebTorrent 主链、附件契约或媒体身份层。
...
2. 禁止把 WebCodecs 生成的 canonical 当成原始文件身份。
...
3. 业务消息不同：仍然保持不同消息事实。
4. 权限不同：禁止泄漏跨权限资产存在性。
5. 肉眼相似：当前不做，禁止伪装成已支持。
6. 转发分享：复用可见源附件绑定的 canonical 资产，不重新上传和转码。
...
11. 禁止把媒体资产身份限制在单个房间里。
12. 禁止把转发 / 分享实现成重新上传或重新生成 canonical。
13. 禁止把“客户端持有原文件证明”作为所有复用场景的前提。
14. 禁止让 `source_hash` 查询成为无权限的全局媒体存在性探针。
...
11. 同一用户在房间 A 上传原始视频，再在房间 B 上传同一原始视频，第二次应复用同一 canonical 资产并创建房间 B 的新消息附件事实。
12. 两位不同用户在互不可见房间上传同一 canonical 视频，后端物理资产仍只能有一份 `content_hash / swarm_id / torrent_info_hash`。
13. 当前用户把自己可见的媒体附件转发到另一个有发送权限的房间时，不重新上传、不重新转码、不重新生成 torrent，只新增新消息和新附件引用。
14. 当前用户看不见的源附件不能被转发，也不能通过 `source_hash` 响应探测出其存在。
15. 源附件删除或失效后，转发路径不能绕过删除事实继续复活已删除 canonical。
...
**媒体物理资产按 canonical 字节全局唯一；上传前用受权限约束的 `source_hash` 复用已有资产；转发分享用可见源附件复用已有资产；上传后用 `content_hash` 精确收口同一 canonical 字节；分发层用 `swarm_id / torrent_info_hash` 复用 WebTorrent 平面；播放器只消费媒体源，不拥有身份与分发真相。**

这条链既能减少重复预处理、重复上传、重复 WebTorrent 种子和重复服务器冷源压力，也明确禁止把房间误当成媒体身份边界，禁止把转发分享误做成重新上传，禁止把 `source_hash` 做成全局存在性探针。
