...
**服务器不能把“发到不同房间”解释成“不同媒体”；房间只拥有消息和附件引用，不拥有媒体资产身份。**
...
5. 转发和分享只能增加业务引用，不能制造第二个 torrent 或第二个 swarm。
...
这条链既能减少重复预处理、重复上传、重复 WebTorrent 种子和重复服务器冷源压力，也不会把房间误当成媒体身份边界，更不会把转发分享误做成重新上传，或把 `source_hash` 做成全局存在性探针。
