...
- 编写/分析代码或维修 bug 时，先从宏观层面看项目结构、关键链路、数据流、性能热点、并发压力和边界约束，再落到局部；源代码默认追求高硬件资源利用率，除非逻辑必须同步，否则优先异步、非阻塞、可背压、可高并发；实现必须保持高性能代码逻辑，默认考虑高并发下的内存、CPU、IO、锁竞争、背压和退场成本；禁止写出低性能代码，禁止写出会主动制造 OOM 风险的无界内存、整文件读入、无限队列、无背压缓存、资源不退场和冗余垃圾代码，禁止为求快写低效率、重复扫描、阻塞热路径或放大资源消耗的代码。
...
- 冒烟测试、浏览器群聊真实体验、前端交互、媒体时间线、自动播放、查看器或页面回归，默认使用 HTTPS，必须联用这三个 CLI skill：`playwright-cli`、`chrome-devtools-cli`、`browser-trace`；辅助按需用 `qa`、`qa-only`、`verification-before-completion`、`systematic-debugging`、`chrome-cdp`、`playwright-dev` 收口验证与根因；禁止只跑其中一条就自称闭环，禁止自造临时浏览器脚本或旁路乱测。
...
- 大任务或小任务验证通过后都自动 `git commit`，除非主人明确禁止；只提交自己本次改动，提交说明用中文详细写明干了什么、为什么做、验证了什么、影响了什么边界。
...
This project is indexed by GitNexus as **koko** (15931 symbols, 29827 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
