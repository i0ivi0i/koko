...
- 简洁是硬约束：行数差距只是过度复杂的信号，重点是代码是否能用更少概念、更直接结构、更低维护成本表达；纯计算和业务规则优先用函数式编程习惯表达：小而纯的函数、不可变数据、组合、显式输入输出。一旦发现实现明显绕远、概念堆叠或资深工程师会判断“这东西过度复杂”，必须重写为极致高效、优雅、足够简单且富有 Unix 哲学智慧的版本，禁止在复杂实现上继续修补，也禁止为函数式而制造抽象或牺牲热路径性能。
