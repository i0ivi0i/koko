...
- 每次写完 spec 或 plan 后，必须执行这个自信循环：“你对当前实现 100% 有信心吗？如果没有，请找出所有可能的底层逻辑漏洞，提出合适的修复方案，然后不断重复这个循环，直到你对新实现在事实上达到 100% 自信为止”；100% 自信必须来自当前实现、调用链、状态机、边界、测试和真实验证证据，禁止停在口头自信。
