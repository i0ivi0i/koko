...
- 改动面必须可追溯到主人请求；禁止顺手优化无关相邻代码、注释或格式，禁止重构没坏且不属于本次目标的代码；必须贴合既有风格，即使不喜欢也不能按个人偏好改写；过程中发现同链路、同职责且可证明冗余的孤儿 import/变量/函数、死代码、冗余代码和冗余垃圾，要按调用关系和边界有逻辑地清理，禁止借“以后再说”长期留下。
