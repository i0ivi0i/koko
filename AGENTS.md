...
- 简洁是硬约束：记住这句原文："If you write 200 lines and it could be 21, rewrite it. Ask yourself: \"Would a senior engineer say this is overcomplicated?\" If yes, simplify." 行数差距只是过度复杂的信号，重点是代码是否能用更少概念、更直接结构、更低维护成本表达；Functional Programming（函数式编程）是一种编程范型，业务逻辑优先按这种范型组织：函数作为主要组织单元，表达式优先于命令式语句，默认不可变，尽量纯函数/引用透明，用组合、高阶函数、代数数据类型和模式匹配表达流程，把 IO、状态变化和副作用隔离到边界。一旦实现绕远、概念堆叠或资深工程师会判断“过度复杂”，必须重写为极致高效、优雅、足够简单且富有 Unix 哲学智慧的版本；禁止把函数式误解成“只做计算”，也禁止为范式制造抽象或牺牲 Rust 所有权、热路径性能和边界清晰度。
...
- 每次写完 spec 或 plan 后，必须逐字执行这个循环：“你对当前实现 100% 有信心吗？如果没有，请找出所有可能的底层逻辑漏洞，提出合适的修复方案，然后不断重复这个循环，直到你对新实现在事实上达到 100% 自信为止。” 这不是一句总结口号，而是强制循环：先问是否事实 100% 自信；否就继续深挖所有可能底层逻辑漏洞、写出合适修复方案、更新 spec/plan，再从头追问；只要仍有未解释漏洞、未验证边界、未闭环状态流或未证明假设，就禁止交付。
...
- 分析代码、编辑代码、查 bug、评估影响范围必须组合使用 GitNexus 与 Serena：GitNexus 先确认索引新鲜并查项目图谱、执行流、调用链、owner 与 blast radius（`list_repos/status/query/context/impact/detect_changes/cypher`），Serena 再做语义级符号搜索、引用关系分析、当前文件/相邻模块精读和精确编辑；后端按纯 Rust、前端按纯 TypeScript 多语言项目处理，纯文本/文件名快扫可用 `rg` 辅助，禁止退化成只 grep 或只看当前标签页。
...
## Discipline
- 项目迭代、功能增删改、代码/逻辑修复、新增/替换代码时，必须眼观六路耳听八方地扫同链路调用点、入口出口、测试、文档、日志和旧兼容面；主动删冗余、假状态、重复逻辑、旧入口、无意义包装、脏工作树、死代码、残留垃圾、无用调用点和旧兼容链，禁止“先留着”“以后再说”的垃圾长期共存。
...
This project is indexed by GitNexus as **koko** (13175 symbols, 24690 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
