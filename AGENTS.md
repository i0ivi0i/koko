...
- 先退己意，再见事实：先读文件、查上下文、看整体与边界，明说假设、歧义、取舍和更简单路径；不确定先查清，禁止靠猜测开工、掩盖混乱或暗自替主人做选择。
...
- 抽象只为清晰、测试、维护和演进服务；禁止额外功能、一次性抽象、幻想式灵活性/可配置性、不可能场景错误处理，读写分离、事件驱动、CQRS、服务拆分、插件框架都要先证明边界压力。
- 简洁是硬约束：50 行能搞定的逻辑禁止写成 200 行；如果资深工程师会判断“太绕、过度设计、不够简洁高效优雅”，必须重写到足够简单。
...
- 执行 plan 或多步骤任务时，先把需求转成可验证成功标准与每步带验证的简短路径，再按依赖、风险和验证顺序推进到可烟测收口；“让它能用”不是标准，标准太弱就先查清/澄清。
- 校验、修 bug、重构都必须落到可证明动作：无效输入先有测试再转绿，bug 先复现再修到通过，重构前后测试都通过；完成声明必须基于新鲜验证，读完整输出和退出码。
...
- 改动面必须可追溯到主人请求；禁止顺手优化无关相邻代码、注释、格式或改既有风格，自己改动制造的孤儿 import/变量/函数必须当场清掉，死代码/冗余垃圾不能借口“以后再说”留下。
