...
describe("前端聊天状态 owner", () => {
  it("应用根聊天状态已经成为唯一 owner，旧根入口已删除", () => {
    const ownerSource = 读取前端源码("应用根/聊天状态.ts");
    ...
    expect(viewportSource).toContain('from "../应用根/聊天状态.js"');
    expect(viewportSource).not.toContain('from "../状态.js"');
    expect(scrollSource).toContain('from "../应用根/聊天状态.js"');
    expect(scrollSource).not.toContain('from "../状态.js"');
    expect(presenterSource).toContain('from "../应用根/聊天状态.js"');
    ...
    expect(fitnessSource).toContain('ownerPath: "frontend/应用根/聊天状态.ts"');
  });
});
