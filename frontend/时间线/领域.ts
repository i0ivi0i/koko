...
/** 附件升级补丁：只允许 patch 附件自身拥有的稳定字段，不允许偷改 kind/宽高等消息成立事实。 */
export type 附件升级补丁 = Partial<Pick<附件快照, "status" | "distribution_hint" | "has_preview_asset" | "preview_asset">>;
...
export type 时间线输入 =
  ...
  | { type: "ATTACHMENT_UPGRADE"; messageId: string; attachmentId: string; patch: 附件升级补丁 };
...
/**
 * 在已有时间线中原地升级指定消息的指定附件槽位。
 * 找不到 messageId 或 attachmentId 时返回原数组引用（幂等空操作）。
 * 附件升级不改变消息条数和顺序，不触发排序和裁剪。
 */
function 应用附件槽位升级(
  current: 消息事件[],
  messageId: string,
  ...
  const idx = current.findIndex((m) => m.message_id === messageId);
  if (idx === -1) return current;
  const msg = current[idx]!;
  const attachments = msg.attachments;
  if (!attachments || attachments.length === 0) return current;
  const attIdx = attachments.findIndex((a) => a.attachment_id === attachmentId);
  if (attIdx === -1) return current;
  const oldAtt = attachments[attIdx]!;
  const newAtt: 附件快照 = { ...oldAtt, ...patch } as 附件快照;
  if (
    oldAtt.status === newAtt.status &&
    oldAtt.distribution_hint === newAtt.distribution_hint &&
    oldAtt.has_preview_asset === newAtt.has_preview_asset &&
    oldAtt.preview_asset === newAtt.preview_asset
  ) {
    return current;
  }
  const newAttachments = [...attachments];
  newAttachments[attIdx] = newAtt;
  const newMsg: 消息事件 = { ...msg, attachments: newAttachments };
 ...
 * - ATTACHMENT_UPGRADE：附件槽位状态升级（pending/processing → ready/failed）
 *
...
export function 推进房间时间线(
  ...
  switch (input.type) {
    ...
    case "ATTACHMENT_UPGRADE":
      return 应用附件槽位升级(current, input.messageId, input.attachmentId, input.patch);
  }
}
